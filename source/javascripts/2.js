...
function FixedAnime() {
  let elemTop = $('#service').offset().top; //#serviceの位置まできたら
...
function setFadeElement() {
  ...
  //出現範囲の指定
  let contentsTop = Math.round($('#contact').offset().top); //要素までの高さ四捨五入した値を取得
  let contentsH = $('#contact').outerHeight(true); //要素の高さを取得
  //2つ目の出現範囲の指定※任意
  let contentsTop2 = Math.round($('#footer').offset().top); //要素までの高さ四捨五入した値を取得
  let contentsH2 = $('#footer').outerHeight(true); //要素の高さを取得
  //出現範囲内に入ったかどうかをチェック
...
if (windowwidth > 768) {
  let responsiveImage = [ //PC用の画像
    {
      src: './images/2/main_01.jpg'
    }, {
      src: './2/main_02.jpg'
    }, {
      src: './2/main_03.jpg'
  ...
  let responsiveImage = [ //タブレットサイズ（768px）以下用の画像
    {
      src: './2/main_01_sp.jpg'
    }, {
      src: './2/main_02_sp.jpg'
    }, {
      src: './2/main_03_sp.jpg'
    }
  ];
}
